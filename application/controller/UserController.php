...
class UserController extends AppController
{
    ...
    public function crudListadmin()
    {
        ...
        $this->pushDataToView = $this->getDefaultResponse();
        $this->pushDataToView[SystemConstant::DATA_LIST_ATT] = $this->userService->findAlladmin($this->getRowPerPage(), $q_parameter);
        $this->pushDataToView[SystemConstant::APP_PAGINATION_ATT] = $this->userService->getTotalPaging();
        jsonResponse($this->pushDataToView);
    }
    ...
    public function crudAdd()
    {
        ...
        if (!empty($jsonData)) { // ถ้า jasonData ไม่ว่าง
            ...
            if ($validator->getValidationErrors()) {
                ...
                if ($lastInsertId) {
                    //create user_role
                    $userRoles = isset($userRolesVal) ? $userRolesVal : null;
