...
class SecurityUtil
{
    ...
    {
        return apache_request_headers(); //ดักข้อมูลใน  headers
    }

    public static function getReqHeaderByAtt($attName = null)//attName ที่ส่งมา มีค่าเป็น apiClient
    {
        $headers = self::getRequestHeaders(); //นำข้อมูลที่จาก header มาเก็บใน $headers
        return $attName != null && !empty($headers) && isset($headers[$attName]) ? FilterUtils::filterVarString($headers[$attName]) : null;// ถ้า attName ไม่มีค่าเป็น null และ headersไม่ว่าง และ header.attName ต้องมีค่า ให้ จัดการค่าให้เป็น string กล่าวคือ ให้ header ที่มีค่า apiClient = default เป็นสตริง
    }
